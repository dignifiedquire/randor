...
module.exports = (limit, logSize, parallelLevel, write, read) => {
  ...
  if (read) {
    // Hack as otherwise the backpressure does not work properly
    // Ref https://github.com/caolan/highland/issues/380
    const rawStore = () => _(fs.createReadStream('store.json').pipe(ndjson.parse()))
    options = rawStore().fork().take(1)
    store = rawStore().fork().drop(1)
  ...
  return _
    ...
    .flatMap((args) => {
      ...
      const execs = ops
              .fork()
              .map((type) => instantiate(OPERATIONS, opts.seed, type))
              .map(exec(ipfs))
